...
async function main() {
  ...
  const keys = ["DATABASE_URL", "SESSION_SECRET", "DOCUSIGN_INTEGRATION_KEY", "DOCUSIGN_USER_ID", "DOCUSIGN_ACCOUNT_ID", "DOCUSIGN_PRIVATE_KEY", "DOCUSIGN_BASE_PATH"];
  ...
  const envelopes = new Map<string, { payload: any; status: string; pdf: Buffer }>();
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if (req.method === "POST" && route.endsWith("/envelopes")) {
        ...
        envelopes.set(envelopeId, { payload: body, status: "sent", pdf: Buffer.from(body.documents[0].documentBase64, "base64") });
  ...
  const post = (handler: (request: NextRequest, context: { params: Promise<{ id: string }> }) => Promise<Response>, id: string) => handler(new NextRequest(`http://localhost/api/intakes/${id}/docusign`, { method: "POST" }), { params: Promise.resolve({ id }) });
  ...
  try {
    ...
    const makeCase = async (withClient = false) => {
      const client = await prisma.client.create({ data: { providerId: provider.id, fullName: "Synthetic DocuSign Client", dob: "1991-02-03", email: "synthetic-docusign@example.invalid", phone: "2025550101" } });
    ...
    console.log("DocuSign HTTP integration passed: JWT, provider/read-only guards, actual recipient/document/tab payload, preserved consent decline, duplicate and uncertain-send protection, sent/delivered/declined/voided audit, invalid response/PDF rejection, completed import/hash/idempotency, protected copies, missing in-app signature gate, and content-change race. All network traffic was loopback; no real envelope was sent.");
  } finally {
