...
export default function DocumentCenter({ intakeId, initialReviewId }: { intakeId: string; initialReviewId?: string }) {
  ...
  return <main className="mx-auto max-w-[1500px] space-y-5 p-4 sm:p-6">
    ...
    {data && <>
      <p className="rounded-xl border bg-white p-3 text-sm text-slate-600">Preparing a blank provider form? <Link className="font-semibold text-brand underline" href={`/adobe?providerId=${data.providerId}`}>Open Adobe preparation tools</Link>. Client review copies remain in this Document Center.</p>
