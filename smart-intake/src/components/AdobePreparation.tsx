...
import AcrobatDesktopWorkflow, { PreparationTemplateStatus } from "./AcrobatDesktopWorkflow";
...
export default function AdobePreparation({ providerId }: { providerId: string }) {
  ...
  function reviewCopy(id: string, sourceId: string) {
    setPreviewId(id); setCompareId(sourceId); setProviderConfirmed(false);
    document.getElementById("adobe-review-heading")?.scrollIntoView({ behavior: "smooth" });
  }
  async function upload() {
    ...
    setPreviewId(saved.id); setDesktopSourceId(saved.id); setCompareId(sourceFileId); setProviderConfirmed(false); changeSelection([saved.id]); await load(); setMessage("Blank form saved and inspected. Continue with Acrobat desktop editing, an optional cloud tool, or mapping review.");
  }
  ...
  return <main className="mx-auto max-w-[1500px] space-y-5 p-4 sm:p-6">
    ...
    {!data ? <p role="status">Loading Adobe tools…</p> : <>
      ...
      <AcrobatDesktopWorkflow key={`${providerId}:${desktopSourceId}`} data={data} sourceId={desktopSourceId} onSourceChange={setDesktopSourceId} disabled={busy} onReview={reviewCopy} onSaved={async (id, sourceId) => { await load(); reviewCopy(id, sourceId); setMessage("Revised PDF selected alongside its source. Check the comparison before mapping."); }} />
      <div className="grid items-start gap-5 lg:grid-cols-2">
        <section className="space-y-3 rounded-xl border bg-white p-4" aria-labelledby="adobe-upload-heading"><h2 id="adobe-upload-heading" className="text-lg font-bold">1. Add a blank form</h2>{data.canWrite && <button className="btn-ghost" disabled={busy} onClick={() => void action(async () => { const saved = await post(base, { action: "create_sample" }); await load(); setPreviewId(saved.id); setDesktopSourceId(saved.id); setCompareId(""); setProviderConfirmed(false); changeSelection([saved.id]); setMessage("Synthetic sample created and selected. Choose an Adobe tool to try it."); })}>Try with a synthetic sample</button>}<p className="text-sm text-slate-600">Upload a source or a copy edited in Acrobat. PDF, DOCX, XLSX, PPTX, TXT, PNG or JPEG; up to 25 MB and 100 PDF pages.</p>
      ...
      <section className="space-y-4 rounded-xl border bg-white p-4" aria-labelledby="adobe-review-heading"><h2 id="adobe-review-heading" className="text-lg font-bold">3. Review originals and prepared copies</h2><label className="block text-sm font-semibold">Saved file<select className="input mt-1 w-full" value={previewId} onChange={e => { setPreviewId(e.target.value); setProviderConfirmed(false); }}><option value="">Choose a saved file</option>{data.files.map(f => <option key={f.id} value={f.id}>{f.name} · {new Date(f.createdAt).toLocaleString()}</option>)}</select></label>
        {preview && <><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-semibold break-all">{preview.name}</p><p className="text-sm text-slate-600">{preview.pageCount || "Unknown"} pages · {size(preview.byteCount)} · {preview.inspection.fieldCount ?? "Unknown"} PDF form fields</p>{preview.inspection.warnings?.map(w => <p key={w} className="text-sm text-amber-900">{w}</p>)}</div><a className="btn-ghost" href={`${base}/files/${preview.id}?download=1`}>Download / open in Acrobat</a></div>
          <button className="btn-secondary" disabled={busy || preview.mimeType !== "application/pdf" || !!preview.inspection.encrypted} onClick={() => { setDesktopSourceId(preview.id); document.getElementById("acrobat-desktop")?.scrollIntoView({ behavior: "smooth" }); }}>Edit this copy in Acrobat</button>
          <PreparationTemplateStatus file={preview} />
          {data.isMaster && preview.promotedTemplateId && <Link className="btn-ghost inline-flex" href={`/master/dashboard?providerId=${providerId}`}>Open packet setup for approval</Link>}
