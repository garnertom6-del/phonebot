...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (intake.client.email) {
    attempts.push(await sendClientLinkEmail(intake.client.email, intake.client.fullName, link, provider!.name, provider!.phone));
  }
  if (intake.client.phone) {
    attempts.push(await sendClientLinkSms(intake.client.phone, link, provider!.name, provider!.phone));
  }
