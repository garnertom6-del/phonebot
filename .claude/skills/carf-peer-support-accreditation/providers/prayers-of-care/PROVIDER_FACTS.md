# Prayers of Care — confirmed facts

Everything here was confirmed by Tom Garner, Owner / Executive Director. Nothing on this page
is inferred. Anything not listed is still unknown and must not be guessed.

| Field | Confirmed | Notes |
|---|---|---|
| Trading name | Prayers of Care | Full legal entity name not yet confirmed |
| Owner / Executive Director | Tom Garner | Sole owner |
| Governance | Sole owner, no board of directors | Owner performs governance functions personally. CARF 1.B is "(Optional)" in the 2026 manual |
| Workforce | Owner plus 4–10 people | Individual names, roles and hire dates not yet collected |
| Operating status | Already serving people | Service history exists — the evidence clock has started |
| Faith-based activity | None | Name-only. No prayer, worship or devotional activity is offered as part of services |
| CARF program sought | Community Integration (3.C, p.185) | Section 3, Core Treatment Program Standards |
| Consultant | Successful Solutions | |

## Still needed before the full packet can be built

- Full legal entity name, and DBA if different
- Physical address, city, state, ZIP; counties actually served
- Main phone; website if there is one
- Exact name of the state peer credential staff hold
- Which EHR the agency documents in
- Payers
- Target survey month
- Date the program started delivering services (drives every due date)
- Shifts that deliver service, sites controlled, vehicles used
- Staff roster: name, role, hire date
- Whether a Program Supervisor is designated, and who
- Whether the Consumer-Run specialty designation (5.D, p.318) is being considered
- Who holds Privacy Officer and Compliance Officer (neither was on the agency's own org chart)
- What independent check exists on the Owner's own work — bank reconciliation, record review,
  and where a complaint about the Owner goes

## Binder progress

| Doc | Title | CARF area | Status |
|---|---|---|---|
| MVV-01 | Mission, Vision and Values | 1.A | Draft — awaiting adoption signature |
| ORG-01 | Organizational Chart and Function Assignment | 1.A, 1.I, 2.A | Draft — awaiting staff roster |
